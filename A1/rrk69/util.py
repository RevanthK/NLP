...
class BigramLanguageModel:
    ...
    def compute_bigram_prob_laplace(self, alpha):
        ...
        for i in range(self.n):
            sum = np.sum(self.bi_prob[i])
            self.bi_prob[i] = (self.bi_prob[i] + alpha) / (sum + (alpha*self.n))
    ...
    def compute_bigram_prob_interpolation(self, beta, alpha):
        ...
        V = np.sum(self.bi_counts)
        unigram_laplace = np.zeros((self.n), float)
        for i in range(self.n):
            unigram_laplace[i] = (np.sum(self.bi_counts[i], axis=0) + alpha) / (V + self.n * alpha)

        for i in range(len(self.bi_prob)):
            self.bi_prob[i] = beta*self.bi_prob[i] + ((1.0 - beta) * unigram_laplace)
